(() => {
  ...
  const createTypewriter = (options = {}) => {
    ...
    const typewriterEffect = async (element) => {
      ...
      if (paragraphID && !Object.prototype.hasOwnProperty.call(originals, paragraphID)) {
        originals[paragraphID] = element.innerHTML;
      }

      if (!paragraphID && !originalElements.has(element)) {
        originalElements.set(element, element.innerHTML);
      }

      const originalHTML = paragraphID
        ? originals[paragraphID]
        : originalElements.get(element);
    ...
    const start = async () => {
      ...
      for (const element of elements) {
        ...
        if (typeof onElementComplete === "function") {
  ...
  const startSectionTitleTypewriter = () => {
    ...
    createTypewriter({
      selector: ".typewriter-title",
      charDelay: 70,
      sentencePauseTime: 180,
      paragraphDelayDefault: 0,
      onElementComplete: (element) => {
        if (element.classList.contains("typewriter-title")) {
          revealSectionBackArrows();
        }
      },
    }).start();
  ...
  window.startSectionTitleTypewriter = startSectionTitleTypewriter;

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", startSectionTitleTypewriter);
  } else {
    startSectionTitleTypewriter();
  }
})();
